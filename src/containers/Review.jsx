...
const Review = ({ productId: productIdProp }) => {
    ...
    useEffect(() => {
        if (productIdProp && productIdProp.productId) {
            const currentProductId = parseInt(productIdProp.productId);

            if (!isNaN(currentProductId)) {
                const relevantReviews = reviews.filter(review => review.product_id === currentProductId);
                setFilteredReviews(relevantReviews);
                setDisplayCount(3);
            } else {
                console.error('Invalid product ID:', productIdProp.productId);
                setFilteredReviews([]);
            }

        } else {
            console.warn("Product prop is missing or invalid.");
            setFilteredReviews([]);
        }
    ...
    const handleShowMore = () => {
        setDisplayCount(prevCount => Math.min(prevCount + 3, filteredReviews.length));
    }
    ...
    return (
        <div className="flex flex-col sm:w-1/2">
            ...
            <div className="my-8">
                <h3 className="mb-8 font-medium">{filteredReviews.length} ความเห็น</h3>

                {/* Review comments section */}
                <div className="flex flex-col gap-8 ">
                    {filteredReviews.length > 0 ? (
                        filteredReviews.slice(0, displayCount).map((review, id) => (
